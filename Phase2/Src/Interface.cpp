...
stringstream Interface::doPostCommand(stringstream& input)
{
	...
	if(word == SIGNUP)
		...
		return doDefaultPriceFilterCommand(input);
	else if (word == SORT)
		return doSortCommand(input);
	else if(word == MANUAL_WEIGHTS)
		return doSetManualWeightsCommand(input);
	else 
...
stringstream Interface::doDefaultPriceFilterCommand(stringstream& input)
{
	checkQuestionMark(input);
	string word;
	input >> word;
	if(word != ACTIVE)
		throw runtime_error(BAD_REQ_ERR);
	input >> word;
	utrip -> setDefaultFilter(word);
	return commandDone();
}

stringstream Interface::doSortCommand(stringstream& input)
{
	checkQuestionMark(input);
	vector<string> argumans = fixer.fixSortArgumans(input);
	utrip -> addSortFilter(argumans[ZERO], argumans[ONE]);
	return commandDone();
}
...
stringstream Interface::doGetCommand(stringstream& input)
{
	...
	if(word == WALLET)
		...
		return doGetManualWeightsCommand();
	else if(word == ESTIMATED_WEIGHTS)
		return doGetEstimatedWeightsCommand();
	else
		throw runtime_error(NOT_FOUND_ERR);
}
...
stringstream Interface::doSetManualWeightsCommand(stringstream& input)
{
	checkQuestionMark(input);
	vector<string>argumans = fixer.fixSetManualWeightArgumans(input);
	if(argumans[ZERO] == FALSE_MSG)
		utrip -> deactiveManualWeights();
	else
	{
		Weights weights(stod(argumans[ONE]), stod(argumans[TWO]), stod(argumans[THREE]),
			stod(argumans[FOUR]), stod(argumans[FIVE]));
		utrip ->setManualWeights(weights);
	}
	return commandDone();
}

stringstream Interface::doGetManualWeightsCommand()
{
	return utrip -> printManualWeights();
}

stringstream Interface::doGetEstimatedWeightsCommand()
{
	return utrip -> printEstimatedWeights();
}