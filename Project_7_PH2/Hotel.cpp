...
stringstream Hotel::generalPrint()
{
	stringstream output;
	output << setprecision(ZERO) << id << SPACE << name << SPACE << star << SPACE << city << SPACE << num_of_rooms ;
	output<< SPACE << fixed << setprecision(DIGITS_AFTER_POINT) << average_cost << endl;
	return output;
}
...
stringstream Hotel::specialPrint()
{
	...
	output << id << endl;
	output << name << endl;
	output << STAR << COLON << SPACE << star << endl;
	output << OVERVIEW << COLON << SPACE << over_view << endl;
	output << AMENITIES << COLON << SPACE << amenities << endl;
	output << CITY << COLON << SPACE << city << endl;
	output << LATITUDE << COLON << SPACE << fixed << setprecision(DIGITS_AFTER_POINT) << latitude << endl;
	output << LONGTITUDE << COLON << SPACE << fixed << setprecision(DIGITS_AFTER_POINT) << longitude << endl;
	output << ROOMS << COLON << SPACE ;
	for(int i = ZERO; i < NUMBER_OF_ROOM_TYPE; i++)
		output << rooms[i].size() << SPACE;
	output << endl << PRICE << COLON << SPACE;
	for(int i = ZERO; i < NUMBER_OF_ROOM_TYPE; i++)
