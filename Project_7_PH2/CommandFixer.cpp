...
vector<string> CommandFixer::fixSortArgumans(stringstream& input)
{
	vector<string> words = readInput(input);
	if(words.size() != FOUR)
		throw runtime_error(BAD_REQ_ERR);
	vector<StringPair> argumans = makePairWords(words);
	sort(argumans.begin(), argumans.end());
	if(argumans[ZERO].first != ORDER || argumans[ONE].first != PROPERTY)
		throw runtime_error(BAD_REQ_ERR);
	vector<string> output = {argumans[ONE].second, argumans[ZERO].second};
	return output;
}

vector<string> CommandFixer::fixSetManualWeightArgumans(stringstream& input)
{
	vector<string> words = readInput(input);
	if(words.size() == TWO)
	{
		if(words[ZERO] != ACTIVE || words[ONE] != FALSE_MSG)
			throw runtime_error(BAD_REQ_ERR);
		return {FALSE_MSG};
	}
	else if(words.size() != TWELVE)
		throw runtime_error(BAD_REQ_ERR);
	vector<StringPair> argumans = makePairWords(words);
	sort(argumans.begin(), argumans.end());
	checkSetManualWeightsValidArgumans(argumans);
	return {argumans[ZERO].second, argumans[THREE].second, argumans[ONE].second,
		argumans[FOUR].second, argumans[TWO].second, argumans[FIVE].second};
}

void CommandFixer::checkSetManualWeightsValidArgumans(vector<StringPair> argumans)
{
	string valids[]={ACTIVE, CLEANLINESS, FACILITIES, LOCATION, STAFF, GET_VALUE_FOR_MONEY};
	for(int i = ZERO; i < argumans.size(); i++)
	{
		if(argumans[i].first != valids[i])
			throw runtime_error(BAD_REQ_ERR);
