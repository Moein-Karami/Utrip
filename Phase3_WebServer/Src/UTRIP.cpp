...
void Utrip::signUp(string email, string username, string pass)
{
	if(logged_in_user != NULL || name_to_user[username] != NULL ||
	 email_to_user[email] != NULL || email == BLANK || username == BLANK || pass == BLANK)
...
void Utrip::fastLogIn(string email)
{
	logged_in_user = email_to_user[email];
}
...
double Utrip::getCurrentMoney()
{
	return logged_in_user -> getCurrentMoney();
}

vector<vector<string> > Utrip::printAllHotels()
{
	checkLoggedInUser();
	if(hotels.size() == ZERO)
	...
	for(auto hotel : chosen_hotels)
		output.push_back(hotel -> generalPrint());
...
string Utrip::getLoggedInUserName()
{
